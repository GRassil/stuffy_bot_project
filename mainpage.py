...
bot = telebot.TeleBot(os.getenv("TOKEN"))  # Создаём объект бота
...
def add_homework(message: types.Message):
    ...
    def add_homework_exam_type(message: types.Message):
        try:
            ...
            kb.add(btn1, btn2, btn3)
            sent = bot.send_message(
                message.chat.id, "ДЗ какого экзамена?", reply_markup=kb
            )
    ...
    def add_homework_finish(message: types.Message):
        nonlocal homework
        answers = homework.right_answers.split()
        try:
            ...
            bot.send_document(
                ...
                reply_markup=kb,
                visible_file_name=f"Вариант номер {homework.hw_id} по {homework.exam_type}",
            )
        except Exception as error:
            logger.error(error)
...
def test_checking_menu(message: types.Message):
    ...
    try:
        with conn:
            user_test = UserTestResult.get_or_none(UserTestResult.teacher_file_id == None)  # Получение пробников, 2 часть которых не проверена
        if user_test:
            kb = types.InlineKeyboardMarkup()
...
def homework_checking_menu(message: types.Message):
    ...
    try:
        with conn:
            user_test = UserHomeworkResult.get_or_none(UserHomeworkResult.teacher_file_id == None,
                                                       UserHomeworkResult.student_file_id != None)  # Получение пробников, 2 часть которых не проверена
        if user_test:
            kb = types.InlineKeyboardMarkup()
...
def registration(message: types.Message):
    ...
    def finish_registration(message: types.Message):
        ...
        try:
            a = bot.get_chat_member(chat_id=-1001846971901, user_id=user.user_id)
            logger.debug(a)
...
def exercise_menu(message: types.Message, page: int = 1, num: int = 1):
    ...
    def select_number(message: types.Message):
        ...
        btns = [types.KeyboardButton(str(i)) for i in range(1, examtype.number_of_questions + 1)]
        kb.add(*btns)
        sent = bot.send_message(
            chat_id=message.chat.id,
...
def send_homework_results(message: types.Message, hw_id: int):
    ...
    if homework.right_answers:
        ...
        if homework.hw_type == 1:
            ...
            for i, (test_answer, user_answer, res) in enumerate(zip(homework_answers, user_answers, user_results_1part)):
                text += f"`{res}{i + 1}. {user_answer} (Ответ: {test_answer})\n`" if res == "❌" \
                    else f"`{res}{i + 1}. {user_answer}\n`"
            bot.send_message(chat_id= message.chat.id,
                             text=text,
        ...
        bot.send_document(
            ...
            visible_file_name=f"{user_homework.user_id} ДЗ {homework.hw_id_id}",
            reply_markup=kb
        )
    else:
        bot.send_message(
